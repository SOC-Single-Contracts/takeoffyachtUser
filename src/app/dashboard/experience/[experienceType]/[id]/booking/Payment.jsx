...
const PaymentForm = ({ isPartialPayment, setIsPartialPayment, bookingDetails, sendTotal }) => {
  ...
  const { id: yachtId, experienceType } = useParams();
      ...
      if (experienceType == "regular-exp") {
        try {
          const url = session?.user?.userid
            ? `${API_BASE_URL}/yacht/get_experience_booking/${bookingData.bookingId}/?user_id=${session?.user?.userid}`
            : `${API_BASE_URL}/yacht/get_experience_booking/${bookingData.bookingId}/`;
      ...
      try {
        ...
        if (experienceType == "regular-exp") {
          if (paymentType === "remaining") {
            endpoint = `${API_BASE_URL}/yacht/exp-capture-remaining-payment/${bookingData.bookingId}/`;
          } else {
            endpoint = `${API_BASE_URL}/yacht/exp-capture-initial-payment/${bookingData.bookingId}/`;
          }
        } else if (experienceType == "f1-exp") {
          if (paymentType === "remaining") {
            endpoint = `${API_BASE_URL}/yacht/exp-capture-remaining-payment/${bookingData.bookingId}/`;
...
const Payment = () => {
  ...
  useEffect(() => {
    ...
    const fetchBookingDetails = async () => {
      ...
      if (experienceType == "regular-exp") {
        try {
          const url = session?.user?.userid
            ? `${API_BASE_URL}/yacht/get_experience_booking/${bookingId}/?user_id=${session?.user?.userid}`
            : `${API_BASE_URL}/yacht/get_experience_booking/${bookingId}/`;
          const response = await fetch(url);
          if (!response.ok) {
  ...
  //test
//   useEffect(()=>{
// console.log("bookingDetails,dueAmountAlltime",bookingDetails,dueAmountAlltime)
//   },[bookingDetails,dueAmountAlltime])
      ...
      <div className='w-full md:w-1/2 space-y-4'>
        <div className='bg-white dark:bg-[#24262F] rounded-xl shadow-md p-6'>
          ...
          <div className='space-y-3'>
            <div className='flex justify-between text-sm'>
              <span>
                {experienceType == "regular-exp" ? bookingDetails?.booking_type === 'date_range' ? (
              ...
              {experienceType == "regular-exp" ? <span className='font-medium'>
                AED {bookingDetails ? bookingDetails.total_cost : dueAmountAlltime}
              </span> : experienceType == "f1-exp" ? "" : ""}
            ...
            <div className='border-t dark:border-gray-600 pt-4 mt-4'>
              <div className='flex justify-between font-semibold text-lg'>
                <span>Total Due</span>
                {experienceType == "regular-exp" ? "" : experienceType == "f1-exp" ? "" : ""}
