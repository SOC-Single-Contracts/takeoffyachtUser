...
const Summary = ({ onNext, initialBookingId }) => {
      ...
      try {
        if (experienceType == "regular-exp") {
          ...
          const url = session?.user?.userid
            ? `${API_BASE_URL}/yacht/get_experience_booking/${currentBookingId}/?user_id=${session?.user?.userid}`
            : `${API_BASE_URL}/yacht/get_experience_booking/${currentBookingId}/`;
          ...
          const activeExtras = details?.extras.filter(extra => extra.quantity > 0);
    ...
    try {
      ...
      const payload = {
        ...
        booking_id: currentBookingId,
         booking_type:  experienceType == "regular-exp" ? "regular" : experienceType == "f1-exp" ? "" :""
  ...
  const calculateUpdatedTotalCost = () => {
    const baseHourlyRate = bookingDetails?.is_new_year_booking
      ? (selectedYacht?.experience?.new_year_price || 0)
      : (selectedYacht?.experience?.per_hour_price || 0);
  ...
  const handleUpdateExtras = async () => {
    try {
      ...
      if (bookingDetails?.booking_type === 'hourly') {
        const hourlyRate = selectedYacht?.experience?.per_hour_price || 0;
        ...
        baseCost = (selectedYacht?.experience?.per_day_price || 0) * days;
      }
  ...
  const handleCopyLink = () => {
    const bookingId = bookingDetails?.id;
    const yachtId = selectedYacht?.experience?.id || bookingData.yachtId;
    const bookingLink = `${window.location.origin}/dashboard/experience/${experienceType}/${yachtId}/booking/?bookingId=${bookingId}`;
          ...
          {/* <TableRow>
            ...
            <TableCell className="font-medium">
              AED {bookingDetails?.end_date
                ? (selectedYacht?.experience?.per_day_price || 0) * (Math.ceil((new Date(bookingDetails?.end_date) - new Date(bookingDetails?.selected_date)) / (1000 * 60 * 60 * 24) + 1))
                : (bookingDetails?.is_new_year_booking
                  ? (selectedYacht?.experience?.new_year_price || 0)
                  : (selectedYacht?.experience?.per_hour_price || 0)) * bookingDetails?.duration_hour
              }
            </TableCell>
          </TableRow> */}
      ...
      <div className="max-w-5xl mx-auto container px-2 space-y-6 mt-8">
        ...
        {/* <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 justify-center place-items-center my-8 max-w-screen-xl mx-auto px-4">
              {!selectedYacht || !selectedYacht?.yacht ? null : (() => {
                ...
                return (
                  <Card
                    key={selectedYacht?.experience?.id}
                    id={`yacht-${selectedYacht?.experience?.id}`}
                  ...
                  >
                    <div className="relative">
                      <Carousel>
                        ...
                        <CarouselDots yId={selectedYacht?.experience?.id} />
                      </Carousel>
                    </div>
                  </Card>
                );
        ...
        <Table className="bg-[#F4F0E4] w-full rounded-lg">
          ...
          <TableBody className="bg-white dark:bg-gray-800 text-xs">
            ...
            <TableRow>
              ...
              <TableCell className="font-medium">
                {experienceType == "regular-exp" ? bookingDetails?.end_date
                  ...
                  : experienceType == "f1-exp" ? bookingDetails?.end_date
                    ? `${Math.ceil((new Date(bookingDetails?.end_date) - new Date(bookingDetails?.selected_date)) / (1000 * 60 * 60 * 24) + 1)} days`
                    : `notprovided`
                    : ""}
