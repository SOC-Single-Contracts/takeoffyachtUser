...
const PaymentForm = ({ isPartialPayment, setIsPartialPayment }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const endpoint = bookingData.remainingCost > 0
        ? `https://api.takeoffyachts.com/yacht/capture-remaining-payment/${bookingData.bookingId}/`
  ...
  return (
    <form onSubmit={handleSubmit} className='w-full space-y-6'>
      ...
      {/* <Button
        type='submit'
        disabled={isProcessing || !stripe || !cardComplete || !bookingData.termsAccepted}
        className='w-full bg-[#BEA355] text-white rounded-full hover:bg-[#A89245] disabled:opacity-50 disabled:cursor-not-allowed h-12'
      >
        {isProcessing ? 'Processing...' : isPartialPayment ? 
          `Pay Initial Amount (AED ${(calculateTotal() * 0.25).toFixed(2)})` : 
        ...
        <div className='bg-white dark:bg-[#24262F] rounded-xl shadow-md p-6'>
          ...
          <div className='space-y-3'>
            ...
            {/* <div className='border-t dark:border-gray-600 pt-3 mt-2'>
              <div className='flex justify-between font-semibold'>
                <span>Total Amount</span>
                <span>AED {bookingDetails ? bookingDetails.total_cost : calculateTotal()}</span>
              </div>
              {isPartialPayment && (
                <>
                  <div className='flex justify-between text-blue-600 dark:text-blue-400 font-semibold mt-2'>
